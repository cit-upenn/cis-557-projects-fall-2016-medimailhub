class MailboxerMessageMailer < Mailboxer::BaseMailer
  ...
  def new_message_email(message,receiver)
    ...
    @receiver = receiver
    set_subject(message)
    attachments["#{@message.attachment.file.filename}"] = File.read(@message.attachment.file.path) if @message.attachment && @message.attachment.file
