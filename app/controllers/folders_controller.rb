...
def create 
   ...
   if @folder.save 
    ...
    if @folder.parent #checking if we have a parent folder on this one 
      ...
      redirect_to '/assets' #if not, redirect back to home page 
    end
   else
    render :action => 'new'
   end
  ...
  def update
    respond_to do |format|
      if @folder.update(folder_params)
        if @folder.parent_id
        format.html { redirect_to browse_path(@folder.parent_id), notice: 'Folder was successfully updated.' }
      else
        format.html { redirect_to '/assets', notice: 'Folder was successfully updated.' }
      end  
